# DEEP V1 - protocol specification

**Name:** Decentralized Extensible Endpoint Protocol.  
**Application and framing version:** 1.  
**Reference implementation:** 1.0.0.  
**Status:** custom specification for testing and interoperability.  
**License:** Apache 2.0.
...
4. The **V1 TLS profile** authenticates the server and protects that stream.
...
## 3. Transport and cryptography profile

The included profile opens TCP to the adapter's `host:port` address. This specification does not assign a DEEP port; the port is always part of the endpoint configuration. Another transport MAY provide the same reliable, ordered, bidirectional stream before TLS is applied.
...
| ALPN | ASCII sequence `deep/1` |
| Server identity | Ed25519 public key in an X.509 certificate |
| Trust | SHA-256 of SubjectPublicKeyInfo DER, pinned by the adapter |
| Ticket resumption / 0-RTT | Disabled |

The client MUST verify the actually negotiated group, ALPN, key type, fingerprint, certificate validity period, and name match against the requested authority. It must not replace this policy with accepting any self-signed certificate. The fingerprint covers the **SPKI** key, not the entire certificate. The DEEP authority is provided as the TLS server name; this profile does not hide SNI.

A self-signed certificate is valid with a trusted fingerprint. A public CA is not required. TLS authentication proves possession of the pinned key. A client certificate is not required.

Content is protected by the AEAD negotiated by TLS 1.3; DEEP does not add its own encryption or MAC. Hybrid negotiation uses ML-KEM-768 for key establishment; **Ed25519 authentication remains classical**. Any connection that does not satisfy this profile is rejected before exchanging DEEP messages. There is no plaintext mode, HTTP mode, or silent fallback to a classical group.
...
| ---: | --- | --- | --- |
| 1 | `HELLO` | 0 | `{"versions":[1]}` |
| 2 | `WELCOME` | 0 | `{"version":1,"max_chunk":65536}` |
...
After completing TLS, the client sends `HELLO`. The `versions` list contains 1 to 16 distinct integers between 1 and 65535. A V1 server selects 1 if present and responds with `WELCOME`. `max_chunk` is exactly 65536 in V1. If there is no common version, the server sends `ERROR` with ID 0 and code `UNSUPPORTED_VERSION`, then closes.

Header version 1 remains mandatory during this negotiation. Including an unknown version in `HELLO` does not authorize unknown framing. The client does not send requests before receiving and validating `WELCOME`.
...
`ERROR` is terminal for the connection. It uses ID 0 during negotiation and the active ID during a request. `code` is 1 to 64 UTF-8 bytes; `message` is at most 1024 UTF-8 bytes. Both are strings and are neither executed nor interpreted as protocol content. Clients must be able to display unknown codes.
...
| `TOO_LARGE` | Resource exceeds the protocol limit. |
...
`HELLO`, ID 0, with `{"versions":[1]}` (16 bytes):

```text
44 45 45 50 01 01 00 00 00 00 00 00 00 00 00 10 00 00 00 00
7b 22 76 65 72 73 69 6f 6e 73 22 3a 5b 31 5d 7d
```

`DATA`, ID 1, metadata `{}`, ASCII body `abc`:

```text
44 45 45 50 01 05 00 00 00 00 00 01 00 00 00 02 00 00 00 03
7b 7d 61 62 63
```
...
```json
{"version":1,"authority":"node.weird"}
```

The program MUST exit with code zero and return exactly one endpoint object on stdout. The stdout limit is 64 KiB and the default resolution timeout is 10 seconds, shortened by an earlier client deadline. Diagnostics may be written to stderr; they never form part of the endpoint. Responses with unknown or duplicate keys, null required fields, or an invalid endpoint are not allowed.
...
`FileHandler` interprets `/` as `/index.txt`, decodes escapes once, and confines access to its root. It rejects empty, hidden, or Windows-unsafe path components, nonempty queries, and nonregular files. This policy belongs to the file provider, not to all possible DEEP resources.
...
Reference implementation local defaults: 1 GiB per resource on the client, 30 seconds per operation/transfer, and 5 seconds for the server's TLS handshake. The server CLI supports 128 connections; the library defaults to 64. These operational limits can be stricter than the framing maximum and are not negotiated as network capabilities.

## 9. Evolution and interoperability

V1 rejects unknown fields, flags, types, and operations. An incompatible extension needs a newly agreed version; it must not be introduced as undocumented optional behavior. An implementation in another language must produce the same bytes, states, and checks, even if its local API differs.

The library name, language, file paths, and CLI commands are not protocol requirements. This specification is distributed to support independent implementations. The Python v0.1 implementation retained in `legacy/` is incompatible with V1 and is not a fallback.
